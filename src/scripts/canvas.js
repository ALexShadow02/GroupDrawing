...
function customMouseUp(event) {
  ...
  if (mode == 'rect') {
    ...
    ctx.arc(
      ...points[0],
      Math.abs(Math.floor(x1 - points[0][0])),
      0,
      Math.PI * 2
    )
    ctx.closePath()
  ...
  if (mode == 'rect') {
    ...
    figure.inOutline = (point) => {
      let [x0, y0] = point
      return (
        x0 <= figure.topPoint[0] + figure.width &&
...
function locateFigure(point) {
  ...
  for (let i = figures.length - 1; i >= 0; i--) {
    ...
    if (figure.type == 'rect') {
      if (
        x0 <= figure.points[0][0] + figure.width &&
...
function fillFigure(figure) {
  if (figure.type == 'rect')
    ctx.fillRect(...figure.points[0], figure.width, figure.height)
