const roomID = window.location.pathname.slice(window.location.pathname.lastIndexOf('/') + 1)
...
socket.onmessage = (mes) => {
    if(mes.data == 'send-img') {
        ...
        let [, user, total] =  mes.data.split(':')
        if(total > 1) createEvent(`${user} has left. Now there are ${total} users in the room`)
